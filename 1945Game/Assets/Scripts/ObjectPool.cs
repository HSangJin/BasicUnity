...
{
    //프리펩과 초기 크기를 받아 풀 초기화

    //생성자에서 프리펩과 초기 크기를 받아 풀을 초기화합니다.
    ...
    //Get메서드는 풀에서 사용가능한 오브젝트를 가져옵니다.
    //풀이 비어있으면 새로운 오브젝트를 생성합니다.
    //가져온 오브젝트를 활성화 상태로 반환합니다.
    //사용이 끝난 오브젝트를 풀로 변환

    //Return 메서드는 사용이 끝난 오브젝트를 비활성화 상태로 풀에 반환합니다.
    //이 클래스는 오브젝트 풀링을 통해 오브젝트 생성과 파괴에 따른 성능 저하를 줄이고, 메모리 관리를 효율적으로 할수있도록 도와줍니다.
